 ...
 * @param {{ stage: string, percent: number, message?: string }} info
 */
 ...
 * @param {ProgressCallback} [opts.onProgress] - Called periodically with stage and 0–100 percent
...
export async function convertEpubToPdf(epubBuffer, opts = {}) {
  ...
  try {
    progress("start", 0, "Writing EPUB to temp file");
    ...
    progress("extract", 10, "Extracted EPUB archive");
    ...
    progress("parse", 18, "Parsed manifest and TOC");

    let spineHtmlCount = 0;
    for (const itemRef of spine) {
      ...
      if (!isHtml) continue;
      const chapterPath = path.resolve(opfDir, item["@_href"]);
      if (await fileExists(chapterPath)) spineHtmlCount += 1;
    }
    ...
    for (const itemRef of spine) {
      ...
      processedHtmlIndex += 1;
      const chapterPct =
        20 +
        (processedHtmlIndex / Math.max(spineHtmlCount, 1)) * 18;
      progress(
        "chapters",
        chapterPct,
        `Processing chapter ${processedHtmlIndex}/${spineHtmlCount}`,
      );
...
<html>
  ...
  <body>
    ...
    progress("merge", 42, "Merged HTML for print");
    ...
    progress("browser", 48, "Chromium ready");

    try {
      const page = await browser.newPage();
      await page.setViewport({ width: screenWidth, height: screenHeight });
      progress("load", 55, "Loading merged document");
      ...
      progress("fonts", 62, "Fonts ready");
      if (settleMs > 0) {
        const tickMs = 500;
        let elapsed = 0;
        while (elapsed < settleMs) {
          await new Promise((resolve) =>
            setTimeout(resolve, Math.min(tickMs, settleMs - elapsed)),
          );
          elapsed += Math.min(tickMs, settleMs - elapsed);
          const settlePct = 62 + (elapsed / settleMs) * 14;
          progress(
            "settle",
            settlePct,
            `Settling layout (${elapsed}/${settleMs} ms)`,
          );
        }
      } else {
        progress("settle", 76, "Skipping settle delay");
      }
      progress("pdf", 80, "Rendering PDF");
      ...
      progress("pdf", 99, "PDF render complete");
